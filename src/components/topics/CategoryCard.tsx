...
export function CategoryCard({ category }: CategoryCardProps) {
  ...
  return (
    <Link to={`/topics/${category.slug}`} className="block group">
      <Card 
        className={cn(
      ...
      >
        ...
        <CardHeader className="pb-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <Badge 
              variant="outline" 
              className={cn("font-medium shrink-0", category.badgeClass)}
            >
              {category.badgeLabel}
            </Badge>
          </div>
        ...
        <CardContent className="pt-0 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1.5">
              {category.subtopics.slice(0, 3).map((subtopic, index) => (
